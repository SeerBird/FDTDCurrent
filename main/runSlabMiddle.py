...
def uniform(r: np.ndarray, t: float) -> ndarray:
    return np.asarray((np.zeros_like(r[0]) + 1-np.exp(-t/10**-13) , np.zeros_like(r[0]), np.zeros_like(r[0])))
...
grid = fdtd.Grid("slabMiddle", (40, 40, 40), 0.0000001)
# assign boundaries
...
grid.run(my_starting_rho, 200, "main/",
