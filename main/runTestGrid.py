...
def uniform(r: np.ndarray, t: float) -> State:
    return State(np.asarray((np.zeros_like(r[0]) + 1000, np.zeros_like(r[0]), np.zeros_like(r[0]))), None, None, None)
...
# assign boundaries
grid[:,:,:] = fdtd.Conductor("bababoi", 10**10, 10**-19, 10**8)
grid[10:30, 10:30, 10:30] = fdtd.Source("boogy", uniform)
